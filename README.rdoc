...
=====
  ...
  class User < AR
    ...
    pie_chart :title => "Gender Ratio", :desc => "Male vs Female", :cache_strategy => "daily" do
      [
        { :label => 'Male', :data => User.male.count },
        { :label => 'Female', :data => User.female.count }
      ]
    end

    bar_chart :title => "Locale Distribution", :desc => "User count for each locale" do
      LOCALES.map do |locale|
        { :label => locale, :data => User.send(locale).count }
      end
    end
    ...
    line_chart :title => "New Users", :desc => "New registered users" do
      (7.days.ago..Time.now).map do |date|
        {
          :label => date ,
          :data => User.where("created_at between ? and ?", date.start_of_the_day, date.end_of_the_day).count
        }
      end
    end
    ...
    table_chart :title => "Latest logged in users" do
      {
        ...
        data => User.order("created_at desc").limit(10)
      }
    end
  end
